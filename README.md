...
第一阶段的可选 `ui-document` feature、`UiBindingManifest<State, Msg>` 和
`zsui-uic check` 已可验证 schema、组件 feature、稳定 ID、属性和绑定；格式、命令及
当前覆盖范围见 [UI Document 文档](docs/ui-document.md)。原生 Viewer 和状态保持仍在
后续切片中完成。
