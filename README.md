...
文本、切换、滑块和滚动容器的带值动作已经通过按控件持有的类型化回调进入显式 Viewer
状态。`scroll` 以一个内容子树、显式内容高度、受控偏移和 number 动作构成闭环，布局会
夹取越界偏移；绑定后的滚动位置在 View 重建后继续保留，不使用全局注册表。完整组件
覆盖、高级控件状态迁移和三平台固定 Viewer 证据仍是 v0.2 的完成门槛。
