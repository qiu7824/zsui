...
Cargo。`zsui-uic handoff` 已可把验证后的文档、绑定 schema、值快照和可选原生 PNG
输出为稳定排序、无时间戳和绝对路径的 AI 交接目录；发布时编译或嵌入仍需完成。文件监听、
...
文本、切换和滑块的带值动作已经通过按控件持有的类型化回调进入显式 Viewer 状态，
不使用全局注册表。完整组件覆盖、通用滚动/高级控件状态迁移、三平台固定 Viewer 证据和
发布期嵌入仍是 v0.2 的完成门槛。
