...
本页图片来自真实 Win32、AppKit 或 Linux 窗口的 smoke、Native Proof CI 或对比
脚本，不是设计稿，也不是共享绘制计划导出的替代图。
These images are captured from real Win32, AppKit or Linux windows by smoke
runs, Native Proof CI or comparison scripts. They are not design mockups or
shared-draw-plan substitutes for final platform surfaces.
...
## One Source, Three Platform Experiences / 一份源码，三平台体验

<table>
  <tr><th>Windows · Win32</th><th>macOS 15 · AppKit</th><th>Ubuntu 24.04 · Linux</th></tr>
  <tr>
    <td><img src="images/component-gallery-preview4-inputs.png" alt="Windows Win32 Gallery"></td>
    <td><img src="platform-proof/macos/gallery-inputs-light.png" alt="macOS AppKit Gallery"></td>
    <td><img src="platform-proof/linux/gallery-inputs-light.png" alt="Linux Gallery"></td>
  </tr>
</table>

Gallery 只声明语义导航、输入和分组。框架在内部选择 Fluent、AppKit 与 Linux 的
组合、度量、文字和图标规则；应用代码不传平台枚举或渲染器。

The Gallery declares semantic navigation, input and grouping once. ZSUI selects
the Fluent, AppKit or Linux composition, metrics, typography and icon rules
internally; application code does not pass a platform enum or renderer.

<table>
  <tr><th>Windows · Win32</th><th>macOS 15 · AppKit</th><th>Ubuntu 24.04 · Linux</th></tr>
  <tr>
    <td><img src="platform-proof/windows/notepad-interaction.png" alt="Windows Notepad interaction"></td>
    <td><img src="platform-proof/macos/notepad-interaction.png" alt="macOS Notepad interaction"></td>
    <td><img src="platform-proof/linux/notepad-interaction.png" alt="Linux Notepad interaction"></td>
  </tr>
</table>

这组三张图片来自同一窗口规格和 Notepad 自动交互场景；最终客户区尺寸仍由平台
宿主决定。结构化报告与图片并列存放
在 `platform-proof/`；完整的每次运行产物继续由 CI 上传。

These images use the same window specification and automated Notepad
interaction; each platform host still determines its final client geometry.
The selected structured reports live beside the images in `platform-proof/`;
CI continues to upload the complete artifacts for every run.
