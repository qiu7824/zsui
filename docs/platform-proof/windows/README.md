...
- `notepad-interaction.png` and `.json`: a 960×640 run of the same scripted
  ...
  evidence records the Segoe UI Variable optical families and ZSUI Rust text
  rasterizer used by semantic UI roles; per-glyph fallback, size, line height
  and weight remain owned by the framework.
