...
- Stack 与 Grid 的固有尺寸递归包含子节点行框、控件最小尺寸、间距和两侧内边距；直接父容器不需要重复声明高度。横向 Stack 按子节点实际获得的宽度计算换行高度，Grid 按实际列宽计算行高。页面和页签面板超过视口时进入垂直滚动，页签条和后续兄弟节点不得被内容覆盖，也不缩小文字或控件硬约束。
