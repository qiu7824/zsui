...
SplitView 文档固定使用两个直接子树：第一个是窗格，第二个是始终存在的内容。开关状态
通过 Boolean 属性/动作闭环保存：

```powershell
cargo run --bin zsui-uic `
  --no-default-features `
  --features ui-document,split-view,button,label `
  -- check examples/ui-documents/split-view.json `
...
SettingsCard 文档声明一个非空语义标题和至少一个内容子树；目标 profile 决定它成为
Fluent 分组卡、AppKit 表单分组或 GTK boxed list，而不是把 Windows 卡片复制到三平台：

```powershell
cargo run --bin zsui-uic `
  --no-default-features `
  --features ui-document,shell `
  -- check examples/ui-documents/settings-card.json `
  --bindings examples/ui-documents/settings-card.bindings.json
```

ItemsRepeater 文档把有限的已实例化子树映射到任意规模的全局索引空间；稀疏可变行高、
视口和选择保持为应用拥有的强类型状态：
...
Image 文档接收一个有界的不可变预乘像素帧，并把缩放与插值保持为平台无关语义：

```powershell
cargo run --bin zsui-uic `
  --no-default-features `
  --features ui-document,image-preview,label `
  -- check examples/ui-documents/image.json `
...
`split_view` 恰好包含两个直接子树，窗格在前、内容在后。`open` 是显式 Boolean 状态；
绑定该属性时必须提供 Boolean `open_change` 动作，窗格外点击或 Esc 会写回 `false`。
`mode` 只接受 `adaptive`、`inline`、`overlay`，`pane_placement` 只接受逻辑方向的
...
`settings_card.title` 是可本地化或绑定的非空 string，直接子节点构成设置内容。
Release runtime 编译到框架现有的 semantic `section`，因此 Windows 使用 Fluent raised
...
实例化窗口，不是完整数据源。可选 `item_heights` 使用 `items_repeater_metric_map`，以稳定
直接子节点 ID 声明有限、正数的逻辑 DP 高度；未声明项目继续使用 `viewport.row_height`
作为估算值。`viewport` 使用 `virtual_list_viewport`，包含偏移、估算行高、可见区间、
实例化区间和滚动方向；绑定该属性时必须提供同类型的 `viewport_change` 动作。
可选 `selected`/`select` 使用 `nullable_integer`/`integer` 的受控闭环。发布运行时复用
框架的 ItemsRepeater；滚动条宽度、边距和最小滑块来自目标平台 profile，滚轮、轨道点击和
滑块拖动统一发出强类型视口动作。文档不包含模板解释器、分页请求、产品数据源或平台滚动
句柄；应用可把视口动作直接交给 `PagedListState::update_viewport`，再生成下一批稳定行子树。

`image.frame` 使用 `nullable_image_frame`，保存不可变的预乘 BGRA 像素、稳定帧 ID 和
尺寸；文档编译器重新校验 `width × height × 4`，并把单帧限制在 32 MiB。`fit` 只接受
`contain`、`cover`、`stretch`，`interpolation` 只接受 `nearest` 或 `smooth`；最终像素
由 Win32 GDI+、AppKit Core Graphics 或 GTK/Cairo 消费，文档不包含平台绘图句柄。
`loading: true` 会保留已有完整帧并启用轻量轮询；空帧显示框架的语义 Image 占位。
频繁变化或大型 PNG 继续由应用的 `ZsImagePreviewState` 在后台合并解码，再把稳定快照
交给 Rust `image_preview` builder，避免 Viewer 文档在每次重载时序列化大型实时像素。

Stack 与 Grid 会递归保留子树的固有尺寸，包括文字行框、控件最小尺寸、节点间距和
容器内边距；直接父节点不需要为嵌套字段重复声明 `height` 或 `min_height`。横向 Stack
先按真实分配宽度计算换行文字高度，再决定整行高度；Grid 先确定实际列宽，再用该宽度
计算换行子树和跨行内容所需的行高。UiDocument 的根 Stack/Grid/List/Tabs 在自然内容
超过窗口时自动成为垂直滚动视口，空间不足不会压缩字形，也不会让后续节点或页签内容
越过所属边界。
...
页签条始终留在自己的边界内，当前页按宽度感知的自然高度测量，内容高度超过页签面板
时只滚动面板内容，不覆盖标签条或后续兄弟节点。窄窗口不会把标签文字压缩到平台最小
宽度以下，超出标签条的部分由标签条裁剪。
...
`scroll` 必须有且只有一个内容子节点。默认使用当前宽度递归测量内容自然高度；可选的
非负 `content_height` 只用于虚拟内容或应用明确掌握的外部内容范围，不能小于实际子树
所需高度。`offset_y` 是可选的非负数值属性；需要在 View 重建后保留位置时，将它绑定到
number 属性，同时把 `scroll` 动作绑定到 number 动作。滚动事件产生的新偏移会更新该
显式属性状态，下一棵 View 树继续使用同一偏移；布局会在窗口缩放或文字重新换行后按
重新测得的范围夹取越界值。
...
受控 SplitView 示例使用同一份窗格/内容树和 Boolean 开关状态，平台负责最终分区与
覆盖层参数：
...
cargo run --bin zsui-viewer `
  --no-default-features `
  --features ui-viewer `
  -- examples/ui-documents/split-view.json `
  --bindings examples/ui-documents/split-view.bindings.json `
...
SettingsCard 示例把绑定标题、换行说明和真实 Button 动作编译到目标平台分组：
...
ItemsRepeater 示例声明十万个逻辑项目，但仅实例化固定窗口中的十行；三行使用稀疏可变
高度，点击返回全局索引，滚轮或滑块拖动返回新的强类型视口：
...
Image 示例用同一个不可变帧展示 Contain、Cover 和 Stretch，并由目标最终光栅后端截图：
...
```rust
let timeline = zsui::message_timeline()
    .message(message)
    .scroll_y(state.timeline_offset);
let composer = zsui::composer("输入消息 / Write a message")
    .draft(state.draft);
let inspector = zsui::inspector_panel("检查器 / Inspector")
    .tab(details_tab);
let shell = zsui::ZsWorkbenchShellSpec::new("工作台 / Workbench", sidebar, composer)
    .timeline(timeline)
    .inspector(inspector);

zsui::workbench_shell(shell)
```

`items_repeater(...)`、`image(frame)` 和 `settings_card(...)` 是对应的公开 Rust
构造器；`ZsItemsRepeaterItemMetric`、`item_metric(s)` 与
`items_repeater_viewport_with_metrics(...)` 提供有界可变行高契约。`virtual_list(...)`、
`image_preview(...)` 与 `section(...)` 继续作为兼容或更底层入口。UiDocument 编译器
调用同一组公开构造器，不维护第二套组件实现。
...
固定 Windows Viewer 热重载证明运行在 `windows-2025` Runner。它在同一个真实 Win32
窗口中把 Workbench revision 1 注入为 revision 2，严格校验四个稳定节点、保留的
`timeline_offset`、删除后明确重置的 `composer_draft`、系统 UI 字体、进程内存和最终
`WM_PRINTCLIENT` PNG。Native UI Proof 运行 `30069326871` 的任务 `89406615062` 已在提交
`9060463` 上通过；固定窗口为 960×640，避免托管桌面把超大顶层窗口夹取到工作区尺寸。
