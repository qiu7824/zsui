...
enables optional GTK4 bindings. Platform-native clipboard, file-dialog and menu
adapters therefore do not enter builds that omit their backend feature. The
clipboard adapters map `ClipboardData::Text`/`Empty` to `NSPasteboard` or
`GdkClipboard`; menu adapters lower the shared `MenuSpec` into backend objects
while keeping native toolkit objects and callback targets out of the public
application API.
